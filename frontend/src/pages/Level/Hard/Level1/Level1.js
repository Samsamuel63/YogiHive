...
export default function Level1() {
  ...
  const [currentPose, setCurrentPose] = useState('Vrischikasana')
    ...
    if (webcamRef.current?.video?.readyState === 4) {
      ...
      try {
        ...
        classification.array().then((data) => {
          ...
          // Redirect to Level 2 when skeleton color is 'rgb(0,255,0)'
          if (skeletonColor === 'rgb(0,255,0)') {
            navigate('/hard/level/2'); // Redirect to Level 2
          }
        });
      } catch (err) {
        console.error(err);
