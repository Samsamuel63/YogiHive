...
export default function Level2() {
  ...
  const [currentPose, setCurrentPose] = useState('Tittibhasana'); // Default Pose for Level 2
  ...
  const get_center_point = (landmarks, left, right) => {
    ...
    const r = tf.gather(landmarks, right, 2);
  ...
  const get_pose_size = (landmarks, torso_size_multiplier = 2.5) => {
    ...
    pose_center = tf.broadcastTo(pose_center, [1, 17, 1]);
  ...
  const landmarks_to_embedding = (landmarks) => {
    ...
    return tf.reshape(landmarks, [1, 30]);
  };
  ...
  const handleFinishLevel = () => {
    navigate('/hard/level/3'); // Navigate to the next level
  };
