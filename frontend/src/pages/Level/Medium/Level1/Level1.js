...
export default function Level1() {
  ...
  const [currentPose, setCurrentPose] = useState('Padmasana')
    ...
    if (webcamRef.current?.video?.readyState === 4) {
      ...
      try {
        ...
        classification.array().then((data) => {
          const classNo = CLASS_NO[currentPose.toLowerCase()];
          ...
          if (skeletonColor === 'rgb(0,255,0)') {
            navigate('/medium/level/2'); // Redirect to Level 2
          }
        });
      } catch (err) {
        console.error(err);
