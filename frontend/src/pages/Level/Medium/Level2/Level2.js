...
export default function Level2() {
  ...
  const [currentPose, setCurrentPose] = useState('Vasisthasana'); // Default Pose for Level 2
      ...
      try {
        ...
        let input = keypoints.map((keypoint) => {
          if (keypoint.score > 0.4) {
            if (!['left_eye', 'right_eye'].includes(keypoint.name)) {
        ...
        classification.array().then((data) => {
          ...
          if (data[0][classNo] > 0.97) {
            ...
            skeletonColor = 'rgb(0,255,0)'; // Success color
          } else {
            flag = false;
            countAudio.pause();
          ...
          // Redirect to next level when skeleton color is 'rgb(0,255,0)'
          if (skeletonColor === 'rgb(0,255,0)') {
            navigate('/medium/level/3'); // Redirect to Level 3
          }
        });
      } catch (err) {
        console.error(err);
  ...
  const handleFinishLevel = () => {
    navigate('/medium/level/3'); // Navigate to the next level
  };
